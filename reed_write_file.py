...
"""
# my_dict = {}
# with open('fizkultura.txt', 'r', encoding='utf-8') as f:
#     for line in f:
#         # Здесь мы получаем название предмета (name), выделяя то, что написано в строке файла fizkultura.txt
#         # до знака :, что реализуется с помощью метода split(':')
#         name, stats = line.split(':')
#         # далее реализуется странная процедура (но работающая), ктр
#         # перебирает все, что за : и отбирает пробелы (' ') и цифры, что и присваивается аргументу elems (это кортеж)
#         elems = [i for i in stats if i == ' ' or (i >= '0' and i <= '9')]
#         print(elems)
          # Здесь суммирование и применение метода map, где функция int применяется к каждому итерируемому элементу
          # списка (второго аргумента)
#         name_sum = sum(map(int, "".join(elems).split()))
#         my_dict[name] = name_sum
# print(f'{my_dict}')
""" Почти тоже, с использованием регулярных выражений (бибилиотека re 
"""
# import re
# # Hаходит  числа от 0 до 9 и знак + означает выбор чисел в составе одна и более цифр подряд
# patter = re.compile('[0-9]+')
# with open('fizkultura.txt', 'r', encoding='utf-8') as f:
#     data = f.read()
#     print(re.findall(patter, data))
...
"""
import json
with open('firma.json', 'w', encoding='utf-8') as fw:
    with open('firma.txt', 'r', encoding='utf-8') as fr:
        # Здесь компрехеншн. Формируем словарь. В качестве ключа line.split()[0],
        # а значение - разница выручки и затрат и это "values"
        profit = {line.split()[0]: int(line.split()[2]) - int(line.split()[3]) for line in fr}
        # Здесь мы формируем кортеж, где выводится profit в виде словаря и ср. зн. профита фирм, где он есть
